...
final class JarURLConnection extends java.net.JarURLConnection
{
    ...
    public InputStream getInputStream() throws IOException {
        ...
        this.connect();
        final InputStream inputStream = this.jarEntryName.isEmpty() ? this.jarFile.getData().getInputStream() : this.jarFile.getInputStream(this.jarEntry);
        if (inputStream == null) {
            this.throwFileNotFound(this.jarEntryName, this.jarFile);
        }
        return inputStream;
    ...
    static JarURLConnection get(final URL url, JarFile jarFile) throws IOException {
        ...
        while ((separator = spec.indexOf("!/", index)) > 0) {
            ...
            final JarEntry jarEntry = jarFile.getJarEntry(entryName.toCharSequence());
            if (jarEntry == null) {
    ...
    static class JarEntryName
    {
        ...
        public static JarEntryName get(final StringSequence spec) {
            return get(spec, 0);
        }
        
        public static JarEntryName get(final StringSequence spec, final int beginIndex) {
